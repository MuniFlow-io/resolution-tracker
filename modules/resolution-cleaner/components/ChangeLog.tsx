...
export function ChangeLog({ entries }: ChangeLogProps) {
  ...
  return (
    <Card className="space-y-3 border-gray-800 bg-gray-900/90">
        ...
        {entries.map((entry) => (
          <li key={`${entry.original_value}-${entry.new_value}-${entry.term_key ?? "null"}`}>
            <p className="text-sm text-gray-300">
              ...
              <span className="ml-2 text-xs text-gray-500">({entry.replacement_count})</span>
            </p>
          </li>
        ))}
      </ul>
      <p className="text-xs text-gray-500">Generated: {new Date(timestamp).toLocaleString()}</p>
    </Card>
  );
}
